...
namespace SquareSnail.Infrastructure
{
    public class ProxyProvider
    {
        ...
        public WebProxy GetProxy(bool isFrontEnd)
        {
            if (isFrontEnd)
                return GetBestRandomProxy();
            else
                return GetWorstRandomProxy();
        }
        ...
        private WebProxy GetRandomProxyByMaxResponseTime()
        {
            if (_fiddlerEnabled)
                return new WebProxy("127.0.0.1:8888");

            var webProxies = GetProxiesByMaxResponseTime();
            var randomProxy = webProxies[new Random().Next(0, webProxies.Count)];

            return randomProxy;
        }
        private WebProxy GetBestRandomProxy()
        {
            if (_fiddlerEnabled)
                return new WebProxy("127.0.0.1:8888");

            var webProxies = GetBestProxies();
            var randomProxy = webProxies[new Random().Next(0, webProxies.Count)];

            return randomProxy;
        }
        private WebProxy GetWorstRandomProxy()
        {
            if (_fiddlerEnabled)
                return new WebProxy("127.0.0.1:8888");

            var webProxies = GetWorstProxies();
            var randomProxy = webProxies[new Random().Next(0, webProxies.Count)];
        ...
        private List<WebProxy> GetBestProxies()
        {
            lock (_locker)
            {
                if (MemoryCache.Default.Get("GetBestProxies") != null)
                    return (List<WebProxy>)MemoryCache.Default.Get("GetBestProxies");

                var limit = int.Parse(ConfigurationManager.AppSettings["BestProxiesLimit"]);
                var proxies = _proxyDataProvider.GetActiveBestProxies(limit, "port", "ip", "username", "password", "response_time");
                var worstResponseTime = proxies[proxies.Count - 1]["response_time"].AsDouble;

                _logger.InfoFormat("got [{0}] proxies from db: [{1}] sec worst response time", proxies.Count, worstResponseTime);
                ...
                MemoryCache.Default.Add("GetBestProxies", webProxies, DateTimeOffset.Now.AddMinutes(1));

                return webProxies;
            }
        }
        private List<WebProxy> GetWorstProxies()
        {
            lock (_locker)
            {
                if (MemoryCache.Default.Get("GetWorstProxies") != null)
                    return (List<WebProxy>)MemoryCache.Default.Get("GetWorstProxies");

                var limit = int.Parse(ConfigurationManager.AppSettings["WorstProxiesLimit"]);
                var proxies = _proxyDataProvider.GetActiveWorstProxies(limit, "port", "ip", "username", "password", "response_time");
                var bestResponseTime = proxies[proxies.Count - 1]["response_time"].AsDouble;

                _logger.InfoFormat("got [{0}] proxies from db: [{1}] sec best response time", proxies.Count, bestResponseTime);
                ...
                MemoryCache.Default.Add("GetWorstProxies", webProxies, DateTimeOffset.Now.AddMinutes(1));
